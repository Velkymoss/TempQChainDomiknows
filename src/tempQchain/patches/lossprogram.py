...
def reverse_sign_grad(parameters, factor=-1.0):
    for parameter in parameters:
        ...
        **kwargs,
        ...
        for data in dataset:
            ...
            if self.opt is not None and loss:
                ...
                self.opt.step()
                iter += 1
            if self.copt is not None and loss and iter > c_warmup_iters and iter - c_update_iter > c_update_freq:
                ...
                if c_lr_decay == 0:
                    ...
                    def update_lr(lr):
                        return c_lr * 1.0 / (1 + c_lr_decay_param * c_update)
                elif c_lr_decay == 1:
                    # in pd code / srl strategy 1
                    # c_lr_decay_param = lr_decay_after
                    def update_lr(lr):
                        return lr * np.sqrt(
                            ((c_update - 1.0) / c_lr_decay_param + 1.0) / (c_update / c_lr_decay_param + 1.0)
                        )
                elif c_lr_decay == 2:
                    # in pd code / srl strategy 2
                    # c_lr_decay_param = lr_decay_after
                    def update_lr(lr):
                        return lr * (((c_update - 1.0) / c_lr_decay_param + 1.0) / (c_update / c_lr_decay_param + 1.0))
                elif c_lr_decay == 3:
                    # in pd code / srl strategy 3
                    # c_lr_decay_param = lr_decay_after
                    assert c_lr_decay_param <= 1.0
        ...
        **kwargs,
        ...
        for data in dataset:
            ...
            if iter < c_warmup_iters:
                ...
                closs, *_ = self.cmodel(output[1])  # <-- Constraint loss calculated here
